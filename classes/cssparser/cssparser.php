<?php
/**
 * Modified by Mathieu to handled "less" rules
 * Parse less rules similar with format:
 * .logo {
 *   .icon("/img/logo.png", 100px, 40px);
 * }
 * which returns
 * array('.logo' => array(
  ...
  function Add($key, $codestr) {
    ...
    if(count($codes) > 0) {
      foreach($codes as $code) {
        ...
        // as less ".icon" mixin rule
        preg_match('/^(.[a-zA-Z0-9_\-]+)\(([^\)]*)\)/', $code, $res);
        if (count($res) === 3) {
          $func = $res[1];
          $args = $res[2];
          $params = explode(',', $args);
          if (count($params) === 3) {
            $styles = array(
              'background' => 'url('.trim(trim($params[0]), "\"").') no-repeat scroll 0 0;',
              'width' => trim($params[1]),
              'height' => trim($params[2]),
              'display' => 'inline-block',
              '-spritifier-dpi' => '@2x'
            );
            foreach($styles as $codekey => $codevalue) {
        ...
        if(count($keys) > 0) {
          foreach($keys as $key) {
            if(strlen($key) > 0) {
              $key = str_replace("\n", "", $key);
              $key = str_replace("\\", "", $key);
              $this->Add(trim($key), trim($codestr));
            }
          }
        }
      }
